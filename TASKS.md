...
- [ ] **Next 3:** Preserve the repository-boundary guard and teach controlled-campaign planning to include the closed transitive set of generator-coupled evidence paths in a unit proposal when every path is already inside sealed task authority. Prove that omitted, unrelated, or authority-exceeding paths still fail closed; then run a fresh one-pod, local-only successor from the unchanged accepted target head to ten accepted outcomes or the next exact safety stop. Preserve Campaign U through AA as diagnostic evidence.
