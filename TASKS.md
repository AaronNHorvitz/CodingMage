...
- [ ] **Task 22.3.3 - Execute and reconcile the ten-task controlled-target campaign**
  ...
  - [x] **Sub-task 22.3.3.3:** Record completed, blocked, deferred, human-decision, and rejected outcomes distinctly and never describe ten outcomes as ten completions unless ten tasks pass every completion gate. Campaign AA's terminal projection records two accepted blockers, zero completions, zero deferrals, zero human decisions, zero rejected proposals, and no integration. See `docs/evidence/sprint-22-controlled-preflight.md`.
  ...
  - [ ] **Sub-task 22.3.3.13:** Reconcile a ready provider's exact changed-path report against the authorized dirty inventory before creating a coordinator commit. Treat a mismatch as bounded provider metadata repair, preserve and resume only the integrity-bound initial session, prohibit additional edits during metadata repair, and create no commit until the report and observed inventory match exactly. Exhausted or contradictory repair must remain a durable fail-closed provider-invalid-output pause with the authorized diagnostic worktree retained; unowned or unobservable paths remain a repository-boundary stop.
...
- [ ] **Next 4:** Implement and verify Sub-task 22.3.3.13 without weakening exact changed-path authority, then run a fresh one-pod, local-only successor from the unchanged accepted target head to ten accepted outcomes or the next exact safety stop. Preserve Campaign U through AB as diagnostic evidence.
