...
- [ ] **Task 28.1.1 - Implement Windows filesystem and repository identity**
  - [x] **Sub-task 28.1.1.1:** Implement volume, file, repository, and executable identity with replacement detection and fail-closed unsupported cases. Pure validation only; native probing stays open. See `docs/evidence/sprint-28-windows-path-identity.md`.
  - [x] **Sub-task 28.1.1.2:** Normalize drive-letter, separator, case, long-path, and supported UNC forms without collapsing distinct identities. Pure validation only; native probing stays open. See `docs/evidence/sprint-28-windows-path-identity.md`.
  - [x] **Sub-task 28.1.1.3:** Reject reserved device names, case collisions, unsafe symlinks, junctions, mount points, and reparse-point replacement. Policy hook only; native enumeration stays open. See `docs/evidence/sprint-28-windows-path-identity.md`.
