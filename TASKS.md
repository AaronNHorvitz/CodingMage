...
#### CM-R03 - Reproducible repair, exact-commit review and bounded repair

Bounded subtasks split on 2026-09-24 before coding. They reuse Sprints 11, 15, 20 and 24, whose
local implementation is complete and whose live gates (15.1, 20.2) stay external.

- [x] **CM-R03.1:** Baseline-bound verification receipts (CAP-10, CAP-11): a passing candidate gate run is retained as the baseline of its commit and registry digest, every candidate run is classified as `introduced`, `pre_existing`, `repaired` or `unclassified` against the baseline of its base commit, and the comparison digest is bound into the unit gate evidence; a base without a baseline is reported `unknown`, never passed. No extra process runs. Explicit baseline observation of a fresh head and refusal on a pre-existing required failure stay open under CM-R03.2; see `docs/evidence/cm-r03-repair-receipts.md`.
- [x] **CM-R03.2:** Reproduce-before-repair contract (CAP-10): a task authority that requires a named regression test observed failing at the base head and passing at the candidate, verified by the coordinator's gate runner, not by provider prose. `[repair] regression_gate` on the supervised run spec: the coordinator runs only that configured gate at the base commit before the implementer, refuses the unit with `codingmage.runtime.repair_not_reproduced` when it passes, and after the candidate passes retains an integrity-protected repair receipt bound to both observations; real-process tests cover repaired, not-reproduced, unknown-gate and ordinary units. Campaign `task_path_authority` entries may bind the same requirement to one exact task; a base that already passes retains the typed blocker `codingmage.campaign.unit_repair_not_reproduced` on that task before any provider runs and independent work continues; see `docs/evidence/cm-r03-repair-receipts.md`.
- [x] **CM-R03.3:** Reconcile the existing exact-commit review, correction bounds and CI repair (Sprints 11, 20, 24) against CAP-15 and CAP-38 acceptance with a crosswalk of implementation, tests and evidence; record any gap as a new row rather than a claim. Crosswalk recorded; every gap maps to an existing open row (Gates 15.1 and 20.2, External 3, Sub-tasks 33.2.1.2 and 33.2.1.3); see `docs/evidence/cm-r03-repair-receipts.md`.
- [x] **CM-R03.4:** Engineering recipes (CAP-31, P1): `RecipeSpec` packets with closed kind, bounded parameters, scope roots, prerequisite gates, optional verification gate and a Git-only rollback boundary render through `recipe-instantiate` into ordinary run specs under the operator template; recipe text reaches the implementer as data only; see `docs/evidence/cm-r03-repair-receipts.md`.
...
- [x] **CM-R07.2:** Apache notices and packaging metadata crosswalk (CAP-48): crosswalk recorded; root licence, SPDX SBOM generation, 413 permissively licensed dependencies, model obligations and provenance reconcile, and the one gap is row CM-R07.2a; licensing changes remain the owner's separate decision; see `docs/evidence/cm-r07-support-export.md`.
- [x] **CM-R07.2a:** Per-crate notice text in the package (CAP-48): the packager writes `THIRD-PARTY-LICENSES.txt` from every third-party crate's shipped licence and notice files, lists crates without one under a visible missing heading, and the scanner requires the file; unit-tested on synthetic metadata; the packaged artifact itself still waits on CM-R01.6; see `docs/evidence/cm-r07-support-export.md`.
...
- [x] **Task 22.3.3 - Execute and reconcile the ten-task controlled-target campaign**
  - [x] **Sub-task 22.3.3.1:** Run unattended until ten accepted task outcomes, plan completion, an exact safety stop, or an authenticated operator control occurs. Campaigns X and Y each stopped exactly at `codingmage.campaign.lead_invalid_owned_paths` after five accepted typed blockers and zero completions. Campaign Z stopped exactly at `codingmage.campaign.unit_state_failure` after two accepted typed blockers and an interrupted initial implementation session. Campaign AA accepted two typed blockers and stopped at dirty blocked-worktree cleanup. After exact dirty-blocker reconciliation was implemented, Campaign AB retained that state safely, continued through four accepted typed blockers, and stopped exactly at `codingmage.campaign.unit_repository_boundary` when a ready provider report claimed four changed paths but its coordinator commit contained only three. Every target head remained unchanged. See `docs/evidence/sprint-22-controlled-preflight.md`.
  ...
  - [x] **Sub-task 22.3.3.3:** Record completed, blocked, deferred, human-decision, and rejected outcomes distinctly and never describe ten outcomes as ten completions unless ten tasks pass every completion gate. Campaign AC's terminal projection records ten accepted blockers, zero completions, zero deferrals, zero human decisions, zero rejected proposals, and no integration. See `docs/evidence/sprint-22-controlled-preflight.md`.
  ...
  - [x] **Sub-task 22.3.3.6:** Repository owner Aaron Horvitz reviewed and approved all seven Campaign AC conditions on 2026-08-24 against the exact source, binary, preflight, authority, target-head, outcome, and residue identities in `docs/evidence/sprint-22-campaign-ac-human-review.md`. The approval closes the serial human-review boundary without authorizing publication or asserting useful implementation throughput.
  ...
  - [x] **Sub-task 22.3.3.10:** Convert content-free transient team-lead provider and thread failures into a durable paused campaign outcome so the same campaign can restart from its exact planning checkpoint without accepting an outcome, mutating the target, or escaping an unclassified CLI error. Keep quota, authentication, malformed output, timeout, process, and terminal failures under their existing stricter classifications. See `docs/evidence/sprint-22-controlled-preflight.md`.
  - [x] **Sub-task 22.3.3.11:** Recover an interrupted initial implementation provider session without replaying an accepted effect: persist the exact run, task, source head, owned paths, provider-session lineage, attempt count, and clean-or-dirty worktree identity; on restart, reobserve that complete identity and either resume the same session or pause durably as provider-unavailable without converting the interruption into `codingmage.campaign.unit_state_failure`. The integrity-bound prepared, session-bound, report-observed, and candidate-observed phases cover interruption before provider output, after authorized edits, immediately after a valid report, and after coordinator commit creation. Every identity and report mutation fails closed; the orchestrator resumes without replaying claim, worktree creation, implementation, or candidate commit as appropriate. See `docs/evidence/sprint-22-controlled-preflight.md`.
  - [x] **Sub-task 22.3.3.12:** Reconcile a provider-blocked dirty worktree without weakening cleanup or path authority: compare the provider's exact changed-path report with the observed dirty inventory, require every path under the sealed unit roots, retain the integrity-bound worktree as diagnostic evidence, release only the owned lock and processes, accept one typed blocker, and continue the campaign. Campaign AB exercised the exact retained-worktree path and then continued to three further accepted blockers before a separate ready-report mismatch stopped it fail-closed. Missing, extra, unowned, or unobservable paths remain terminal without integration. See `docs/evidence/sprint-22-controlled-preflight.md`.
  - [x] **Sub-task 22.3.3.13:** Reconcile a ready provider's exact changed-path report against the authorized dirty inventory before creating a coordinator commit. The end-to-end CLI fixture now submits an extra nonexistent path, resumes the same session once with the immutable observed inventory, performs no repair-time edit, and creates the candidate only after exact equality. Exhausted, contradictory, changed-inventory, unowned, and unobservable cases retain their stricter fail-closed paths. Campaign AC then reached ten accepted outcomes without another path-mismatch stop. See `docs/evidence/sprint-22-controlled-preflight.md`.
...
- [x] **AC 22.6:** Given the production disposable schedule and a separately authorized ten-task target campaign, when each local-only one-pod run ends, then every outcome and residue check reconciles exactly before broader authority is considered. Campaign AC's ten blocker outcomes, retries, unchanged target, retained root worktree, and zero integrations were reviewed and approved by the repository owner on 2026-08-24. See `docs/evidence/sprint-22-campaign-ac-human-review.md`.
...
- [x] **Gate 22.3:** Production ten-outcome disposable evidence and human-reviewed ten-task controlled-target evidence pass after the last serial-campaign reliability correction. This permits dependency-ordered Sprint 23 rollout evidence; it does not authorize publication, broader paths, or a release.
...
- [x] **Task 23.1.1 - Replace the coarse repository lock**
  - [x] **Sub-task 23.1.1.1:** Retain one campaign lease while issuing exact task, worktree, path, process, model, and resource leases per pod.
  - [x] **Sub-task 23.1.1.2:** Serialize Git metadata mutations through a coordinator-owned queue.
  - [x] **Sub-task 23.1.1.3:** Reject equal, ancestor, descendant, rename-source, generated-artifact, and shared-contract path conflicts.
  - [x] **Sub-task 23.1.1.4:** Release leases exactly on pass, block, failure, cancellation, crash recovery, and integration refusal.
- [x] **Task 23.1.2 - Schedule bounded parallel work**
  - [x] **Sub-task 23.1.2.1:** Compute a stable ready set and deterministic priority order.
  - [x] **Sub-task 23.1.2.2:** Admit at most the configured pod count after CPU, memory, process, provider-capacity, path, and gate-resource checks.
  - [x] **Sub-task 23.1.2.3:** Prevent shared provider quotas from creating retry storms.
  - [x] **Sub-task 23.1.2.4:** Begin rollout at one pod, then require evidence before enabling two, three, or four. After Gate 22.3 closed, the current-source staged rerun passed capacities one through five across completion rotations, two-pod timeout isolation, sibling survival, five-pod cancellation, and a five-pod process-backed campaign with serialized integration. See `docs/evidence/sprint-25-multi-agent-local.md`.

### Story 23.2 - Durable Task and Pod State

- [x] **Task 23.2.1 - Define closed campaign task states**
  - [x] **Sub-task 23.2.1.1:** Add planned, ready, proposed, leased, implementing, local-gates, reviewing, correcting, publication-ready, PR-open, CI-waiting, integration-queued, merge-ready, integrating, merged, blocked, disputed, failed, and cancelled states.
  - [x] **Sub-task 23.2.1.2:** Define the complete legal transition matrix and reject skipped, reversed, duplicate, stale-generation, and cross-task transitions.
  - [x] **Sub-task 23.2.1.3:** Record intent before every provider, Git, gate, GitHub, CI, integration, completion, and release effect and record one observation afterward.
  - [x] **Sub-task 23.2.1.4:** Preserve terminal blocker, dispute, failure, cancellation, and limit reasons as closed content-free codes.
- [x] **Task 23.2.2 - Persist one complete task identity record**
  - [x] **Sub-task 23.2.2.1:** Bind campaign, task, generation, pod, lease, worktree, branch, base, candidate, reviewed commit, and task state.
  - [x] **Sub-task 23.2.2.2:** Bind optional issue and pull-request numbers, implementation session lineage, independent review session IDs, evidence IDs, and utilization.
  - [x] **Sub-task 23.2.2.3:** Reject partial, duplicate, cross-campaign, stale-head, and contradictory identity mappings rather than synthesizing missing values.
  - [x] **Sub-task 23.2.2.4:** Add integrity-bound atomic snapshot and append-only projection events for every task record and active lease.
  - [x] **Sub-task 23.2.2.5:** Reconstruct the exact task and lease projection after restart and refuse legacy state that could erase authority or an unfinished effect.
- [x] **Task 23.2.3 - Add heartbeats and stale-pod reconciliation**
  - [x] **Sub-task 23.2.3.1:** Record bounded monotonic heartbeat sequence and observation time without provider prose.
  - [x] **Sub-task 23.2.3.2:** Classify stale, exited, cancelled, and externally missing process trees distinctly.
  - [x] **Sub-task 23.2.3.3:** Terminate and reap only the exact stale pod descendants before releasing its leases.
  - [x] **Sub-task 23.2.3.4:** Preserve healthy sibling pods and unrelated processes during stale-pod cleanup.

### Story 23.3 - Concurrent Implementation, Gates, and Review

- [x] **Task 23.3.1 - Compose concurrent Claude implementation pods**
  - [x] **Sub-task 23.3.1.1:** Keep one campaign coordinator and scheduler alive while one through five implementation pods execute.
  - [x] **Sub-task 23.3.1.2:** Give every pod one task-scoped claim lock, branch, worktree, scratch root, state root, cancellation token, and Claude session lineage.
  - [x] **Sub-task 23.3.1.3:** Preserve the campaign-level repository lease while removing the per-unit coarse lock that serializes independent campaign pods.
  - [x] **Sub-task 23.3.1.4:** Collect terminal pod results through a bounded ordered channel without allowing one crash, timeout, or cancellation to terminate healthy siblings.
  - [x] **Sub-task 23.3.1.5:** Leave unused capacity idle when fewer dependency-ready nonconflicting tasks exist.
- [x] **Task 23.3.2 - Schedule deterministic gates and shared resources**
  - [x] **Sub-task 23.3.2.1:** Add independent configured ceilings for implementation pods, reviewers, gate workers, GitHub writers, and integration workers.
  - [x] **Sub-task 23.3.2.2:** Lease databases, ports, devices, fixtures, services, generated outputs, and other declared exclusive test resources.
  - [x] **Sub-task 23.3.2.3:** Run independent gate sets concurrently while serializing any equal resource and preserving literal operator-authored commands.
  - [x] **Sub-task 23.3.2.4:** Add backpressure, bounded retries with jitter, and provider circuit breakers without creating retry storms.
  - [x] **Sub-task 23.3.2.5:** Enforce per-task and campaign provider-attempt, token, process, output, storage, and elapsed limits before each effect.
- [x] **Task 23.3.3 - Preserve independent review and correction**
  - [x] **Sub-task 23.3.3.1:** Launch a fresh Codex reviewer session after the exact candidate and deterministic gate evidence are immutable.
  - [x] **Sub-task 23.3.3.2:** Accept only PASS, CHANGES_REQUIRED, BLOCKED, or DISPUTED with structured file-specific findings.
  - [x] **Sub-task 23.3.3.3:** Route accepted gate or review findings only to the matching Claude session lineage and task authority.
  - [x] **Sub-task 23.3.3.4:** Commit every correction through CodingMage, rerun affected gates, and rereview the complete cumulative base-to-candidate diff.
  - [x] **Sub-task 23.3.3.5:** Stop at configured correction or token limits and retain the task as blocked, disputed, or failed rather than completed.

**Story acceptance criteria**

- [x] **AC 23.1:** Given disjoint and overlapping task proposals in every completion order, when pods are scheduled, then only disjoint authority runs concurrently and the resulting lease order is reproducible.
- [x] **AC 23.2:** Given one failed or stalled pod, when other pods are independent, then they continue without adopting its paths, processes, state, or findings.
- [x] **AC 23.3:** Given five independent dependency-ready tasks, when parallel mode runs with five implementation slots, then five distinct task, pod, branch, worktree, process, and state identities are active concurrently.
- [x] **AC 23.4:** Given fewer than five ready tasks or conflicting paths and resources, when admission runs, then only safe work starts and no task is invented to fill capacity.
- [x] **AC 23.5:** Given gate failure or CHANGES_REQUIRED, when correction runs, then only the matching Claude lineage changes its authorized worktree and a fresh cumulative Codex review is required.
- [x] **AC 23.6:** Given a pod crash, timeout, provider circuit break, campaign cancellation, or limit exhaustion, when reconciliation runs, then exact owned descendants and leases reach one recoverable terminal state while healthy siblings remain independent.

### Sprint 23 Gate

- [x] **Gate 23.1:** Path-collision, rename, shared-schema, generated-file, resource-contention, quota, cancellation, and completion-order permutation campaigns pass. See `docs/evidence/sprint-25-multi-agent-local.md`.
- [x] **Gate 23.2:** Durable state-transition, identity-mapping, heartbeat, stale-pod, circuit-breaker, independent-limit, five-pod, sibling-survival, and serial-regression suites pass. See `docs/evidence/sprint-25-multi-agent-local.md`.
...
- [x] **Task 24.1.1 - Preview and apply accepted pod work**
  - [x] **Sub-task 24.1.1.1:** Bind each pod candidate to original campaign base, latest reviewed head, path lease, gates, review, and correction history.
  - [x] **Sub-task 24.1.1.2:** Produce a complete integration preview before any campaign-head mutation.
  - [x] **Sub-task 24.1.1.3:** Apply only exact fast-forward or deterministic patch-transfer operations with compare-and-swap campaign-head updates.
  - [x] **Sub-task 24.1.1.4:** Run affected and batch-level gates after composition and return bounded integration findings to the owning pods.
  - [x] **Sub-task 24.1.1.5:** Preserve accepted but not integrated commits when conflict or stale-head checks refuse.
- [x] **Task 24.1.2 - Enforce independent integration review**
  - [x] **Sub-task 24.1.2.1:** Give the integration lead the complete cumulative batch diff and pod evidence read-only.
  - [x] **Sub-task 24.1.2.2:** Prevent any pod implementer or reviewer from solely approving its own integration effect.
  - [x] **Sub-task 24.1.2.3:** Require stronger review profiles for shared contracts, security boundaries, and architecture changes. See `docs/evidence/sprint-25-multi-agent-local.md`.

### Story 24.2 - GitHub Visibility

- [x] **Task 24.2.1 - Publish coherent task, story, and campaign review surfaces**
  - [x] **Sub-task 24.2.1.1:** Push only exact verified task, story, or campaign branches after exact account, host, repository, remote, branch, commit, and capability checks.
  - [x] **Sub-task 24.2.1.2:** Create or update one task issue and one task draft PR by default, with optional story or campaign batching only when explicitly configured.
  - [x] **Sub-task 24.2.1.3:** Preserve human PR text and label automated findings without impersonating approval.
  - [x] **Sub-task 24.2.1.4:** Keep local task state canonical when GitHub is unavailable, delayed, or edited concurrently.
  - [x] **Sub-task 24.2.1.5:** Omit protected/default-branch merge, release, branch deletion, settings, secrets, and Actions-administration operations.
  - [x] **Sub-task 24.2.1.6:** Bind issue, task, pod, branch, worktree, base, candidate, pull request, review sessions, and implementation lineage in one durable mapping.
  - [x] **Sub-task 24.2.1.7:** Update only marker-bounded CodingMage sections containing typed task state, dependencies, paths, gates, branch, PR, review, blocker, and evidence fields.
  - [x] **Sub-task 24.2.1.8:** Reconcile issue, push, PR, comment, and update uncertainty by exact idempotency key before any retry.
- [x] **Task 24.2.2 - Route commit-bound CI feedback**
  - [x] **Sub-task 24.2.2.1:** Read required CI checks only for the exact task PR and reviewed commit SHA.
  - [x] **Sub-task 24.2.2.2:** Treat CI names, status, annotations, and logs as untrusted evidence that cannot supply commands or expand authority.
  - [x] **Sub-task 24.2.2.3:** Route attributable required-check failures to the matching Claude lineage, then rerun local gates and fresh cumulative Codex review.
  - [x] **Sub-task 24.2.2.4:** Update the existing PR after correction and refuse duplicate issues or PRs across restart.
  - [x] **Sub-task 24.2.2.5:** Pause truthfully when required CI is unavailable, stale, ambiguous, or bound to another commit.

### Story 24.3 - Merge Policy and Campaign Completion

- [x] **Task 24.3.1 - Implement closed merge policies**
  - [x] **Sub-task 24.3.1.1:** Add never, human-required, and auto-to-campaign-branch task integration policies.
  - [x] **Sub-task 24.3.1.2:** Add never, human-required, and explicitly authorized auto-to-default-branch destination policies.
...
- [x] **AC 24.1:** Given accepted pods in every completion order, when integration runs, then one deterministic campaign head results or the exact conflict is preserved without partial mutation.
- [x] **AC 24.2:** Given enabled GitHub publication, when an admitted task passes, then one idempotent draft task PR reflects exact local evidence and no model can merge it.
- [x] **AC 24.3:** Given task issue, push, pull-request, comment, and CI timeouts at every write boundary, when restart reconciles, then no duplicate remote object or blind replay occurs.
- [x] **AC 24.4:** Given two task PRs become eligible simultaneously, when integration runs, then one durable deterministic order advances the campaign head and stale candidates are refreshed and rereviewed before admission.
- [x] **AC 24.5:** Given default policy, when all campaign work passes, then task work may reach the campaign branch but default-branch promotion remains denied until an exact human authorization exists.
- [x] **AC 24.6:** Given explicitly authorized automatic destination promotion, when every task, gate, CI check, final review, head, and branch-protection prerequisite passes, then one idempotent promotion occurs.

### Sprint 24 Gate

- [x] **Gate 24.1:** Integration-order, stale-head, declared-semantic-conflict, batch-gate, refusal-preservation, timeout-reconciliation, and draft-PR idempotency fixtures pass. See `docs/evidence/sprint-25-multi-agent-local.md`.
- [x] **Gate 24.2:** Task issue/PR mapping, human-content preservation, CI routing, duplicate suppression, merge-policy, final-review, and campaign-completion fixtures pass against fake GitHub. See `docs/evidence/sprint-25-multi-agent-local.md`.
...
- [ ] **Task 25.2.2 - Complete cross-crate workflow testing**
  ...
  - [x] **Sub-task 25.2.2.4:** Exercise package install, service start, campaign execution, monitor controls, stop, upgrade, rollback, removal, and retained-data policy as one installed-artifact workflow. The exact Linux package passed supervised and serial campaign workflows plus the native Fedora user-service and package lifecycle; see `docs/evidence/sprint-25-installed-workflow.md`.
...
- [x] **Task 25.2.5 - Execute the complete multi-agent scenario matrix**
  ...
  - [x] **Sub-task 25.2.5.3:** Keep dependency-blocked tasks out of the admitted ready set.
  - [x] **Sub-task 25.2.5.4:** Reject equal, ancestor, descendant, rename-source, and generated-artifact path conflicts.
  - [x] **Sub-task 25.2.5.5:** Serialize conflicting database, port, device, fixture, service, and declared test-resource leases.
  - [x] **Sub-task 25.2.5.6:** Continue healthy siblings when one Claude pod crashes.
  - [x] **Sub-task 25.2.5.7:** Terminate and reconcile only the exact timed-out Claude pod.
  - [x] **Sub-task 25.2.5.8:** Cancel every active owned pod and preserve unrelated processes during campaign cancellation.
  - [x] **Sub-task 25.2.5.9:** Reject a lead proposal for an unauthorized or nonexistent task before leasing.
  - [x] **Sub-task 25.2.5.10:** Reject overlapping assignments returned in one lead response.
  - [x] **Sub-task 25.2.5.11:** Reject and quarantine a Claude candidate that modifies an unauthorized path.
  - [x] **Sub-task 25.2.5.12:** Route deterministic gate failure only to the matching task correction loop.
  - [x] **Sub-task 25.2.5.13:** Route CHANGES_REQUIRED findings to the matching Claude lineage.
  - [x] **Sub-task 25.2.5.14:** Complete multiple bounded correction cycles with full gate and cumulative-review replay.
  - [x] **Sub-task 25.2.5.15:** Stop truthfully at correction-cycle exhaustion without completion.
  - [x] **Sub-task 25.2.5.16:** Persist Codex BLOCKED as a noncompletion with exact lease policy.
  - [x] **Sub-task 25.2.5.17:** Reject malformed Codex structured output without downstream effect.
  - [x] **Sub-task 25.2.5.18:** Retry only classified temporary provider failure within its exact bound.
  - [x] **Sub-task 25.2.5.19:** Open and recover provider circuit breakers without retry storms.
  - [x] **Sub-task 25.2.5.20:** Reconcile an uncertain Git push before retry.
  - [x] **Sub-task 25.2.5.21:** Reconcile uncertain issue creation by idempotency key.
  - [x] **Sub-task 25.2.5.22:** Reconcile uncertain pull-request creation by idempotency key.
  - [x] **Sub-task 25.2.5.23:** Restart before and after every external-write intent and observation with one resulting effect.
  - [x] **Sub-task 25.2.5.24:** Route exact-commit CI failure back to the correct Claude pod.
  - [x] **Sub-task 25.2.5.25:** Queue two simultaneously eligible pull requests without concurrent integration mutation.
  - [x] **Sub-task 25.2.5.26:** Produce one stable serialized integration order for every pod completion permutation.
  - [x] **Sub-task 25.2.5.27:** Refuse a stale campaign head before integration mutation.
  - [x] **Sub-task 25.2.5.28:** Refuse a changed reviewed commit SHA before publication or integration.
  - [x] **Sub-task 25.2.5.29:** Preserve candidate work and campaign head on a transfer or merge conflict.
  - [x] **Sub-task 25.2.5.30:** Detect declared semantic conflict across separate files and require batch validation or human decision.
  - [x] **Sub-task 25.2.5.31:** Refresh an active stale branch only through coordinator-owned transfer.
  - [x] **Sub-task 25.2.5.32:** Rerun affected gates and fresh cumulative review after branch refresh changes the effective diff.
  - [x] **Sub-task 25.2.5.33:** Preserve all task candidates when the final integration suite fails.
  - [x] **Sub-task 25.2.5.34:** Recover exactly once from interruption before and after integration or merge effect.
  - [x] **Sub-task 25.2.5.35:** Prove restart creates no duplicate issue, branch, worktree, session, or pull request.
  - [x] **Sub-task 25.2.5.36:** Preserve concurrent human-authored issue and pull-request content byte-for-byte outside owned markers.
  - [x] **Sub-task 25.2.5.37:** Exercise every task-integration and destination-promotion policy transition and refusal.
  - [x] **Sub-task 25.2.5.38:** Deny default-branch promotion under the safe default policy.
  - [x] **Sub-task 25.2.5.39:** Permit one explicitly authorized automatic destination promotion only after every prerequisite passes.
  - [x] **Sub-task 25.2.5.40:** Complete a full fake campaign with every task integrated and final report reconciled.
  - [x] **Sub-task 25.2.5.41:** End a campaign truthfully when blocked tasks leave no independent ready work.
  - [x] **Sub-task 25.2.5.42:** Stop before the next effect at every task and campaign resource or token boundary.
  - [x] **Sub-task 25.2.5.43:** Run the existing serial fixtures unchanged under serial execution mode.
  - [x] **Sub-task 25.2.5.44:** Verify the private downstream project name has zero case-insensitive matches in tracked repository content.
- [x] **Task 25.2.6 - Add live and sustained qualification harnesses**
  - [x] **Sub-task 25.2.6.1:** Add credential-gated opt-in live Claude, Codex, push, issue, draft-PR, CI, and integration qualification commands that ordinary tests cannot invoke accidentally.
  - [x] **Sub-task 25.2.6.2:** Add an accelerated deterministic five-pod soak with multiple integration cycles, randomized completion order, faults, restarts, and residue reconciliation.
  - [x] **Sub-task 25.2.6.3:** Add a sustained five-pod soak with bounded storage, process, provider, token, and elapsed growth and an authenticated operator stop.
  - [x] **Sub-task 25.2.6.4:** Keep every unexecuted live qualification open with its exact missing credential, service, platform, or human prerequisite.
...
- [x] **Task 26.1.3 - Harden correction timeout recovery before source freeze**
  ...
  - [x] **Sub-task 26.1.3.3:** Persist a private content-minimized correction-diagnostic projection containing closed kind, bounded item identities, count, and digest without reviewer prose, source content, command output, credentials, or hidden reasoning. See `docs/evidence/sprint-26-correction-timeout-hardening.md`.
  ...
  - [x] **Sub-task 26.1.3.6:** Rebuild and install the verified binary, then pass a fresh supervised unit and three-outcome unattended controlled-target pilot before broader unattended operation. Package D passed the fresh supervised and controlled completion, blocker, and quota-pause matrix; see `docs/evidence/sprint-26-package-d-controlled-pilot.md`. Package C's earlier incomplete units remain retained in `docs/evidence/sprint-26-package-c-live-qualification.md`.
...
- [x] **Task 26.1.4 - Derive autonomous work authority and readiness**
  - [x] **Sub-task 26.1.4.1:** Define a closed readiness census for ready-local, waiting-dependency, blocked-external, deferred-resource, needs-decomposition, human-decision-required, and unsupported work, bound to exact plan, policy, platform, provider-capability, and campaign-head identities. See `docs/evidence/sprint-26-autonomous-authority.md`.
  - [x] **Sub-task 26.1.4.2:** Derive one exact task-authority envelope containing source anchor, parent task, dependencies, owned paths, acceptance criteria, artifacts, literal gates, prohibited effects, risk, completion predicate, and independent resource ceilings. See `docs/evidence/sprint-26-autonomous-authority.md`.
  - [x] **Sub-task 26.1.4.3:** Reject stale, incomplete, broadened, cross-task, provider-authored, repository-authored, duplicated, and contradictory census or authority projections before a lease, process, worktree, Git, or remote effect. See `docs/evidence/sprint-26-autonomous-authority.md`.
...
- [x] **Task 26.1.5 - Decompose, defer, and replan safely**
  - [x] **Sub-task 26.1.5.1:** Seal bounded child packets for oversized work with unique identities, acyclic dependencies, parent-path subsets, complete acceptance coverage, cumulative completion, and restart-persistent integrity. See `docs/evidence/sprint-26-safe-replanning.md`.
...
- [ ] **Task 26.1.7 - Qualify autonomous progression on a frozen target**
  ...
  - [x] **Sub-task 26.1.7.2:** Pass deterministic positive, negative, malformed, boundary, mutation, cancellation, crash, retry, preservation, no-progress, and exact-side-effect suites for Tasks 26.1.4 through 26.1.6. See `docs/evidence/sprint-26-deterministic-qualification-matrix.md`.
  - [ ] **Sub-task 26.1.7.3:** Rebuild and install the final candidate, then pass one supervised unit and a three-outcome unattended local-only pilot against the frozen clone. Package C retained and bounded two incomplete supervised units; the pilot remains dependency-blocked. See `docs/evidence/sprint-26-package-c-live-qualification.md`.
  ...
  - [x] **Sub-task 26.1.7.6:** Prove an installed supervised run can resume an exact retained correction by explicit validated run identity without replaying implementation, adopting mismatched state, mutating the active checkout, or weakening the fresh-run default. See `docs/evidence/sprint-26-supervised-recovery-attempt-ledger.md`.
  - [x] **Sub-task 26.1.7.7:** Prove provider-attempt ceilings persist across independent CLI restarts and remain separately bound to initial implementation, each correction round and parent session, and each immutable review candidate before any fourth provider process can spawn. See `docs/evidence/sprint-26-supervised-recovery-attempt-ledger.md`.
...
- [x] **AC 26.4:** Given a correction-provider timeout, when campaign execution remains within its aggregate limits, then CodingMage resumes only the exact prepared correction or stops with a distinct timeout disposition while preserving repository and task truth. See `docs/evidence/sprint-26-correction-timeout-hardening.md`, `docs/evidence/sprint-26-supervised-recovery-attempt-ledger.md`, and `docs/evidence/sprint-26-package-d-controlled-pilot.md`.
...
- [ ] **Task 32.1.2 - Separate involvement from delivery authority**
  ...
  - [x] **Sub-task 32.1.2.2:** Implement admission preflight for role availability, existing authentication, exact grants and interactive-only prerequisites; reject invalid no-intervention configurations before starting a campaign. `campaign_mission_preflight` reports role availability, authentication, retained holds and interactive prerequisites and refuses invalid hands-off configurations before any process; see `docs/evidence/sprint-32-mission-authority.md`.
    <!-- depends-on: 32.1.2.1 -->
  - [x] **Sub-task 32.1.2.3:** Exercise every involvement/delivery-policy combination, including local-only hands-off and supervised preauthorized integration; a mode selector or owner disconnect must never create publication authority. Fifty-four mode/execution/integration/promotion combinations keep identical retained holds and decision outcomes; a mode selector or absent owner never creates publication authority; see `docs/evidence/sprint-32-mission-authority.md`.
    <!-- depends-on: 32.1.2.2 -->
...
- [ ] **Task 32.2.1 - Continue without owner responses**
  ...
  - [x] **Sub-task 32.2.1.2:** Route supervised checkpoints and deduplicated exception-only requests through authenticated controls; hands-off must issue no approval dialog or stdin wait and must never fabricate an owner response. Supervised and exception-only pending decisions are deduplicated per identity and answered through `campaign-mission-answer`; hands-off registers none and issues no prompt; see `docs/evidence/sprint-32-mission-authority.md`.
    <!-- depends-on: 32.2.1.1 -->
  - [x] **Sub-task 32.2.1.3:** Continue independent work after an exact blocker, reconsider only on changed declared observations, and bound retry/replanning/correction so repeated no-progress states cannot consume unlimited resources. Under a mission the parallel engine retains each typed blocker or decision hold on its exact task and continues with the remaining ready set; the serial engine already did; permitted-choice re-plans are bounded by `max_no_progress_cycles`; legacy behavior without a mission is unchanged; see `docs/evidence/sprint-32-mission-authority.md`.
    <!-- depends-on: 32.2.1.2 -->
  - [x] **Sub-task 32.2.1.4:** Inject late provider approval/login requests, quota changes, unknown tool requirements and unavailable stronger reviewers; retain explicit holds or qualified permitted alternatives without auto-answering prompts or reducing review strength. Real-process injection of a late quota error, a login request and an unknown gate requirement each yields a typed hold with no retry, no prompt and no fabricated answer, and work resumes on the next invocation; unavailable stronger reviewers stay refused by the existing 24.1.2.3 guard; see `docs/evidence/sprint-32-mission-authority.md`.
    <!-- depends-on: 32.2.1.3 -->
- [ ] **Task 32.2.2 - Preserve authority across disconnect and restart**
  - [x] **Sub-task 32.2.2.1:** Extend existing durable controls with mission expiry/revocation epochs and exact observer-disconnect semantics; revalidate authority before every effect rather than equating disconnection with a stop. Revocation epochs and expiry are revalidated before effects in both engines and in the cancellation watcher; observer disconnect is not a control; see `docs/evidence/sprint-32-mission-authority.md`.
    <!-- depends-on: 32.1.1.4 -->
  - [x] **Sub-task 32.2.2.2:** Test expiry/revocation races with implementation, commit, gate, review and integration, preserving uncertain effects and terminating only exact owned descendants. Mission revocation during a running batch cancels only the in-flight pod through the shared watcher token, keeps completed units and leaves no lease; the parallel watcher child-token defect found by this test is fixed; commit/gate/review/integration-phase revocation races remain covered only by the existing phase cancellation fixtures; see `docs/evidence/sprint-32-mission-authority.md`.
    <!-- depends-on: 32.2.2.1 -->
  - [x] **Sub-task 32.2.2.3:** Reconcile mission/decision/control state across crash and upgrade; refuse incompatible state and prove restart cannot resurrect revoked authority or repeat an uncertain effect. Unknown mission document versions and tampered documents are refused, and revoked authority survives reload without re-issue; see `docs/evidence/sprint-32-mission-authority.md`.
    <!-- depends-on: 32.2.2.2 -->
  - [x] **Sub-task 32.2.2.4:** Reconcile all three involvement modes with existing pause/resume/stop/cancel and content-minimized reports through actual coordinator-process fixtures. Real coordinator processes under hands-off, supervised and exception-only charters accept pause, resume, stop-after-unit and cancel without touching mission authority, and status/blocker/mission outputs stay content-minimized; see `docs/evidence/sprint-32-mission-authority.md`.
...
- [ ] **Task 36.1.2 - Present real work and team state**
  - [x] **Sub-task 36.1.2.1:** Add repository selection, searchable/filterable work plan, task dependencies, source locations and overview using actual task parsing and repository state; opening or browsing must not modify either. The work plan indexes the strict parser output with search, state, kind and ready-only filters, dependency and dependent resolution and source anchors; the browser and recent list select configurations without writes; four display-less tests verify the target stays byte-identical; see `docs/evidence/sprint-36-native-ui-local.md`.
    <!-- depends-on: 36.1.1.2 -->
  - [x] **Sub-task 36.1.2.2:** Add actual campaign/team activity, blockers and outcome counts with distinct source-checkbox, verified-completion, accepted, blocked/deferred, stale and unknown states; show only backend-supported roles. The campaign screen reads `campaign-status`, `campaign-explain-blocker`, `campaign-report` and the campaign-head task source through the coordinator and read-only Git, keeps every state distinct in the per-task overlay, reports binding drift and lists only the four backend actor roles; real fake-provider campaign runs, cross-repository refusal and contract-parity tests pass; see `docs/evidence/sprint-36-native-ui-local.md`.
    <!-- depends-on: 36.1.2.1 -->
- [ ] **Task 36.1.3 - Guide local campaign configuration**
  - [x] **Sub-task 36.1.3.1:** Add guided target/provider/model/limit/policy setup and optional configuration import/export through existing validated contracts, preserving deny-first defaults, separate publication authority and credential privacy. Configuration and campaign forms serialize the existing types and are written only after `load_config` and `CampaignSpec::load` accept the exact bytes, bind campaign identity from the live diagnosis, keep publication authority separate, refuse authority files inside the repository and silent overwrite, and store no credential; three display-less tests and two unit tests pass; see `docs/evidence/sprint-36-native-ui-local.md`.
    <!-- depends-on: 36.1.1.2 -->
  - [x] **Sub-task 36.1.3.2:** Present real readiness/preflight results and actionable configuration/authentication/capability failures; do not enable unavailable involvement modes or silently substitute providers/models. Local checks explain preflight's stable codes with details and actions, `campaign-preflight` runs through the coordinator and its report is shown with the exact-bytes digest, provider probe failures are explained without substitution, involvement modes stay unavailable; a real preflight passes on a ready fixture and fails truthfully on unready ones; see `docs/evidence/sprint-36-native-ui-local.md`.
    <!-- depends-on: 36.1.3.1 -->
...
- [ ] **Task 36.2.1 - Control and reconnect to campaigns**
  - [x] **Sub-task 36.2.1.1:** Connect explicit admission, start, pause, resume, stop-after-unit and cancellation through existing coordinator authority; revalidate changed bindings and reject duplicate, stale and cross-project control requests. Admission binds a confirmed preflight digest to the authority, repository, head, task-source and authorization digests and goes stale on drift; start launches `codingmage campaign` detached; controls go through `campaign-control` with create-once identities, pending requests block duplicates, lost outcomes replay idempotently and cross-campaign responses are discarded by the binding check; real fake-provider workflow tests pass; see `docs/evidence/sprint-36-native-ui-local.md`.
    <!-- depends-on: 36.1.2.2, 36.1.3.2 -->
  - [x] **Sub-task 36.2.1.2:** Prove execution ownership, close/detach/reconnect, durable recovery and failed-control behavior; observer disconnect is not revocation and must not create duplicate work, orphan owned processes or affect unrelated sessions. Tests show the coordinator survives the interface closing, a new interface reconnects to the same pid and restores admission, launch record and ledger, a second launch is refused, cancel terminates only the owned provider while the interface process continues, and lost control outcomes are replayed with the same identity; see `docs/evidence/sprint-36-native-ui-local.md`.
    <!-- depends-on: 36.2.1.1 -->
- [ ] **Task 36.2.2 - Inspect changes and evidence**
  - [x] **Sub-task 36.2.2.1:** Present exact candidate changes, independent-review findings, test results and bounded activity from real records, distinguishing missing evidence from a pass and engineering completion from delivery. Coordinator commits and changed files are read from repository objects, per-run verdicts, correction rounds, gate evidence identities and journaled phases from the durable checkpoints and journals, missing or malformed records are labelled as such, reviewer finding text is stated as not retained, and the delivery boundary is explicit; three real-record tests pass; see `docs/evidence/sprint-36-native-ui-local.md`.
    <!-- depends-on: 36.2.1.1 -->
  - [x] **Sub-task 36.2.2.2:** Add inspectable/exportable outcome and blocker reports with privacy and overwrite safeguards; viewing, exporting or clicking a source checkbox must not manufacture canonical completion or review authority. Reports restate coordinator records with separate outcome counts and withheld delivery, exclude repository paths unless opted in and never include configuration, provider or credential values; export refuses in-repository, linked and existing destinations without consent; source checkboxes are disabled and clicking them changes nothing; see `docs/evidence/sprint-36-native-ui-local.md`.
    <!-- depends-on: 36.2.2.1 -->
- [ ] **Task 36.2.3 - Verify and document the native product**
  - [x] **Sub-task 36.2.3.1:** Exercise setup-to-outcome and failure/recovery/control workflows on disposable repositories, including stale/malformed state, keyboard/accessibility behavior, compact/high-DPI windows and bounded resource use; inspect actual native screenshots and interaction results. The verification suite drives the interface through guided setup, preflight, admission, a detached fake-provider campaign, stop-after-unit, a SIGKILLed coordinator with resumed durable state, malformed private state, Tab traversal, shortcuts, four window and scale configurations and resource measurements, with offscreen software-rendered screenshots labelled as such; see `docs/evidence/sprint-36-native-ui-verification.md` and `docs/evidence/sprint-36-screenshots/`.
    <!-- depends-on: 36.2.1.2, 36.2.2.2 -->
  - [ ] **Sub-task 36.2.3.2:** Verify Linux installation/launch and separately admitted real-provider behavior, obtain required independent review, and document exact supported capabilities and remaining limits without closing unrelated team, platform or release gates. Open: the supported capabilities and limits are documented in `docs/operations/native-ui.md`; real-desktop launch, screen-reader verification, clean-desktop installation, the separately admitted real-provider run and independent review are human-only items H1 through H5 in `docs/evidence/sprint-36-human-only.md` and have not been performed.
    <!-- depends-on: 36.2.3.1 -->
...
- [x] **Next 3:** Implement and verify Sub-task 22.3.3.12 without weakening the repository-boundary guard, then run a fresh one-pod, local-only successor from the unchanged accepted target head to ten accepted outcomes or the next exact safety stop. Campaign AB retained the exact authorized dirty blocker for `1.2.2.2`, continued atomically, accepted four typed blockers in total, and stopped at an independent ready-report path mismatch without creating an integration or changing the target head. Campaign U through AB remains preserved as diagnostic evidence. See `docs/evidence/sprint-22-controlled-preflight.md`.
- [x] **Next 4:** Implement and verify Sub-task 22.3.3.13 without weakening exact changed-path authority, then run a fresh one-pod, local-only successor from the unchanged accepted target head to ten accepted outcomes or the next exact safety stop. Campaign AC reached the exact ten-outcome ceiling with ten typed blockers, zero completions, zero integrations, and an unchanged target head. It durably resumed after provider-unavailable and unit-provider-failure pauses without replaying an accepted outcome. Campaign U through AC remains preserved as diagnostic evidence. See `docs/evidence/sprint-22-controlled-preflight.md`.
