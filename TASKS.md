...
- [x] **CM-R03.2:** Reproduce-before-repair contract (CAP-10): a task authority that requires a named regression test observed failing at the base head and passing at the candidate, verified by the coordinator's gate runner, not by provider prose. `[repair] regression_gate` on the supervised run spec: the coordinator runs only that configured gate at the base commit before the implementer, refuses the unit with `codingmage.runtime.repair_not_reproduced` when it passes, and after the candidate passes retains an integrity-protected repair receipt bound to both observations; real-process tests cover repaired, not-reproduced, unknown-gate and ordinary units. Campaign task authority does not yet carry the requirement; see `docs/evidence/cm-r03-repair-receipts.md`.
