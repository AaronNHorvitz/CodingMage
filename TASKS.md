...
- [ ] **CM-R07.2a:** Per-crate notice text in the package (CAP-48): make the release packager collect each dependency's licence and copyright text from the locked graph into the packaged notices so MIT, BSD and Zlib notice conditions are met by the artifact, with a scanner check; needs a constructed candidate to prove, which CM-R01.6 currently blocks.
