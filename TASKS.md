...
- [ ] **Task 22.3.3 - Execute and reconcile the ten-task controlled-target campaign**
  ...
  - [ ] **Sub-task 22.3.3.11:** Recover an interrupted initial implementation provider session without replaying an accepted effect: persist the exact run, task, source head, owned paths, provider-session lineage, attempt count, and clean-or-dirty worktree identity; on restart, reobserve that complete identity and either resume the same session or pause durably as provider-unavailable without converting the interruption into `codingmage.campaign.unit_state_failure`. Prove interruption before provider output, after authorized edits, and immediately before a valid report; reject every missing, substituted, reordered, or widened identity and preserve the unchanged target head.
