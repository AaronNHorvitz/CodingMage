...
def license_texts(package: dict[str, object]) -> list[tuple[str, str]]:
    """Return the (file name, text) pairs of licence and notice files shipped in one crate."""
    manifest = package.get("manifest_path")
    if not isinstance(manifest, str) or not manifest:
        return []
    crate_root = Path(manifest).parent
    candidates: list[Path] = []
    declared = package.get("license_file")
    if isinstance(declared, str) and declared:
        candidates.append(crate_root / declared)
    try:
        entries = sorted(crate_root.iterdir(), key=lambda item: item.name)
    except OSError:
        entries = []
    for entry in entries:
        if entry.is_file() and entry.name.upper().startswith(LICENSE_FILE_PREFIXES):
            candidates.append(entry)
    texts: list[tuple[str, str]] = []
    seen: set[Path] = set()
    for candidate in candidates:
        resolved = candidate.resolve() if candidate.exists() else candidate
        if resolved in seen or not candidate.is_file() or candidate.is_symlink():
            continue
        seen.add(resolved)
        if candidate.stat().st_size > MAX_LICENSE_TEXT_BYTES:
            continue
        try:
            texts.append((candidate.name, candidate.read_text(encoding="utf-8")))
        except (OSError, UnicodeDecodeError):
            continue
    return texts


def write_third_party_licenses(destination: Path, cargo_metadata: dict[str, object]) -> dict[str, int]:
    """Concatenate every third-party crate's licence and notice text into one packaged file.

    Workspace crates are skipped. A crate whose sources carry no licence file is listed under a
    `MISSING LICENSE TEXT` heading with its declared expression so the gap is visible rather
    than silently satisfied. Returns counts of included and missing crates.
    """
    workspace = set(cargo_metadata.get("workspace_members", []))
    sections: list[str] = []
    missing: list[str] = []
    included = 0
    for package in sorted(
        ...
        if package.get("id") in workspace or not package.get("source"):
            continue
        expression = package.get("license") or "NOASSERTION"
        texts = license_texts(package)
        heading = f"==== {package['name']} {package['version']} ({expression}) ===="
        if not texts:
    ...
    document = [
        ...
        "",
        "Generated by scripts/package_release.py from the exact locked dependency graph. Each",
        "section reproduces the licence and notice files shipped in the crate's sources.",
        "",
    ]
    if missing:
        document.append("MISSING LICENSE TEXT (declared expression only; no file shipped in the crate):")
        document.extend(f"- {entry}" for entry in missing)
        document.append("")
    document.extend(sections)
    destination.write_text("\n".join(document) + "\n", encoding="utf-8")
    return {"included": included, "missing": len(missing)}
...
def package(output: Path, review_record: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="codingmage-package-") as temporary:
        ...
        write_third_party_licenses(
            layout / "share" / "doc" / "codingmage" / "THIRD-PARTY-LICENSES.txt", cargo_metadata
        )
