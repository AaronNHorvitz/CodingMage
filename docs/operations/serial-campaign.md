...
Status and checkpoint-journal schemas reject unknown fields and omit prompts, source text,
filenames, provider prose, command output, unrestricted environment values, credentials, and hidden
...
orchestration boundary and never collapse into a generic provider failure. Other terminal runtime
categories remain distinct as `unit_spec_failure`, `unit_authority_failure`, `unit_plan_failure`,
`unit_process_failure`, `unit_state_failure`, `unit_orchestration_failure`,
`unit_campaign_failure`, or `unit_integration_failure`; CodingMage does not merge them into one
ambiguous internal code. Each path clears the durable active-unit marker, releases the pod lease,
...
A release failure is authoritative only when the preceding unit operation succeeded. Final outcome
...
- Campaign checkpoints, accepted-head recovery, and exact correction-session recovery are
  ...
  counts, active identity, dispositions, accepted outcomes, and every configured ceiling. Successful
