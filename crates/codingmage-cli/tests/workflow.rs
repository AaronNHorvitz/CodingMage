...
if "{ 3 }" in source:
    if "correction-round-one-resumed" not in calls:
        ...
        print(json.dumps({"type": "result", "is_error": True, "subtype": "request_timeout"}))
        raise SystemExit(0)
    else:
        assert "--resume" in sys.argv
        assert "remove any temporary probe or scratch artifacts" in packet
...
fn serial_campaign_reports_repeated_correction_timeout_distinctly() {
    ...
    fs::write(target.join("src/lib.rs"), "pub fn value() -> u8 { 1 }\n").unwrap();
    git(&target, &["add", "src/lib.rs"]);
    git(&target, &["commit", "-m", "add timeout fixture"]);
    let original_head = git_output(&target, &["rev-parse", "HEAD"]);
    let original_tasks = fs::read(target.join("TASKS.md")).unwrap();
    let original_source = fs::read(target.join("src/lib.rs")).unwrap();
...
if "--help" in sys.argv:
    ...
    raise SystemExit(0)
root = Path(__file__).parent
log = root / "timeout-claude.log"
path = Path("src/lib.rs")
source = path.read_text(encoding="utf-8")
with log.open("a", encoding="utf-8") as stream:
    stream.write("correction-timeout\n" if "{ 2 }" in source else "implementation\n")
if "{ 2 }" in source:
    assert "--resume" in sys.argv or "--session-id" in sys.argv
    print(json.dumps({"type": "result", "is_error": True, "subtype": "request_timeout"}))
    raise SystemExit(0)
path.write_text("pub fn value() -> u8 { 2 }\n", encoding="utf-8")
print(json.dumps({
...
"#,
    ...
    let config = fixture.root.join("config/timeout-campaign.toml");
    let scratch = fixture.root.join("timeout-scratch");
    let state = fixture.root.join("timeout-state");
    assert!(
        Fixture::command(&[
            "init",
            "--repo",
    ...
    let doctor = Fixture::command(&["doctor", "--config", config.to_str().unwrap()]);
    let diagnosis: serde_json::Value = serde_json::from_slice(&doctor.stdout).unwrap();
    let campaign = fixture.root.join("timeout-campaign.toml");
    fs::write(
        &campaign,
        format!(
            r#"version = 3
...
max_units = 1
implementer_authentication = "existing_login"
campaign_branch = "codingmage/timeout-campaign"
allowed_paths = ["src"]
denied_paths = []
protected_branches = ["main"]
publication = "local_only"
...
"#,
    ...
    assert_eq!(outcome["stop_reason"], "provider_timeout");
    assert_eq!(
        outcome["blocker_code"],
        "codingmage.campaign.unit_provider_timeout"
    );
    assert_eq!(outcome["completed_units"], 0);
    let calls = fs::read_to_string(fixture.root.join("timeout-claude.log")).unwrap();
    assert_eq!(
        calls
            .lines()
            .filter(|line| *line == "implementation")
            .count(),
        1
    );
    assert_eq!(
        calls
            .lines()
            .filter(|line| *line == "correction-timeout")
            .count(),
        3
    );
    assert_eq!(git_output(&target, &["rev-parse", "HEAD"]), original_head);
    ...
    assert_eq!(status["outcomes"]["accepted"], 0);
}
